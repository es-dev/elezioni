    ...
    public class SedeElettoraleViewModel : Library.Template.MVVM.TemplateViewModel<SedeElettoraleDto, SedeElettoraleItem>
    {
        ...
        public override void Load(int skip, int take)
        {
            try
            {
                ...
                var objs = wcf.LoadSediElettorali(skip, take);
                Load(objs);
            }
            catch (Exception ex)
            {
        ...
        public override bool Save(object model, bool creating)
        {
            try
            {
                ...
                {
                    var wcf = new WcfService.Service();
                    var obj = (SedeElettoraleDto)model;
                    bool performed = false;
                    if (creating)
                    {
                        var newObj = wcf.CreateSedeElettorale(obj);
                        performed = (newObj != null);
                        if (performed)
                            obj.Id = newObj.Id;
                    }
                    else //updating
                        performed = wcf.UpdateSedeElettorale(obj);
                    return performed;
                }
            }
            catch (Exception ex)
        ...
        public override bool Delete(object model)
        {
            try
            {
                if (model != null)
                {
                    var wcf = new WcfService.Service();
                    var obj = (SedeElettoraleDto)model;
                    bool performed = wcf.DeleteSedeElettorale(obj);
                    return performed;
                }
            }
            catch (Exception ex)
        ...
        public override object Read(object Id)
        {
            ...
            {
                var wcf = new WcfService.Service();
                var obj = wcf.ReadSedeElettorale(Id);
                return obj;
            }
            catch (Exception ex)
            {
